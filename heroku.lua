...
find_item = function(url)
  value = string.match(url, "^https?://([^/]+)%.herokuapp%.com/$")
  type_ = "site"
  if value then
    item_type = type_
    item_value = value
...
allowed = function(url, parenturl)
  local appname = string.match(url, "^https?://([^/]+)%.herokuapp%.com")
  if appname then
    if appname == item_value then
      return true
    else
      discovered_items["site:" .. appname] = true
      return false
    end
  end
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url) and status_code < 300 then
    html = read_file(file)
    check("https://" .. item_value .. ".herokuapp.com/robots.txt")
    check("https://" .. item_value .. ".herokuapp.com/favicon.ico")
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["heroku-w3mw2b4rqqcimqj"] = discovered_items,
    ["urls-ibzuolcgwrkb7uc"] = discovered_outlinks
  }) do
